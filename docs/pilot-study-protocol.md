...
Flask commit is the first eligible candidate; its selection report is committed
before image preparation. Neither is yet a mutation-adequacy study set. Before
interpreting outcomes, freeze at least six eligible `study` subjects from at
least three maintainer communities, selected using the same rules. This is
still exploratory evidence, not a representative sample.
