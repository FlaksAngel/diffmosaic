...
- role: `calibration`, `candidate`, or `study`;
...
it must not produce or support an experimental claim. Promote a candidate to
`study` only after its trusted image and resolved identity are recorded.

The initial manifest includes three third-party candidates from two independent
maintainer communities. Before interpreting outcomes, expand the frozen `study`
set to at least six projects from at least three communities, selected using the
same rules. This is still exploratory evidence, not a representative sample.
...
4. explicit permission under the project license to analyse the source; and
5. a fixed trusted Docker image for mutation execution.

Exclude generated code, commits that only change documentation or tests,
projects whose tests require credentials or network access, and any project
whose licensing conditions are unclear. Record exclusions and the reason in a
later corpus version rather than replacing failed candidates silently.

## Procedure

1. Freeze the manifest and validate it in CI.
2. Create a coverage JSON artefact in each project's approved isolated build.
3. Run `analyze` and preserve the JSON output.
4. Run `mutate-plan` with a pre-declared candidate limit.
